...
const ussd_session = new EventEmitter();
...
ussd_session.query = function(string, callback, end) {
    ...
    let encoded = ussd_session.encode(string);

    let session = this;
    let execute_job = this.modem.execute('AT+CUSD=1,"'+encoded+'",15', function(response, escape_char) {
...
ussd_session.isResponseCompleted = function() {
    let args = this.modem.parseResponse(this.partial_data);
...
ussd_session.responseCompleted = function() {
    ...
    if(this.modem.ussd_pdu && args[2] == '15')
        let response = pdu.decode7Bit(args[1]);
    else if(this.modem.ussd_pdu)
        let response = pdu.decode16Bit(args[1]);
    else
        let response = args[1];
